...
export const getStorageDetails = (
  ...
  switch (type) {
    case "images":
      return totalStorage.image?.size ?? 0;

    case "documents":
      return totalStorage.document?.size ?? 0;

    case "media":
      const videoSize = totalStorage.video?.size ?? 0;
      const audioSize = totalStorage.audio?.size ?? 0;
      return videoSize + audioSize;

    case "others":
      return totalStorage.other?.size ?? 0;
